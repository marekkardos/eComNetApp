...
@Component({
  ...
  template: `
    <div class="bg-white rounded-2xl shadow-xl p-8">
      ...
      <!-- Google Sign In -->
      <button mat-stroked-button class="w-full py-3" type="button" (click)="onGoogleLogin()" [disabled]="isProcessingOAuth()">
        <div class="flex items-center justify-center gap-2">
          ...
          <span>{{ isProcessingOAuth() ? 'Signing in...' : 'Sign in with Google' }}</span>
        </div>
      </button>

      @if (oauthError()) {
        <div class="bg-red-50 border border-red-200 rounded-lg p-3 mt-3">
          <p class="text-red-600 text-sm">{{ oauthError() }}</p>
        </div>
      }
...
export class LoginComponent implements OnInit {
  ...
  isProcessingOAuth = signal(false);
  oauthError = signal<string | null>(null);
  ...
  ngOnInit(): void {
    ...
    if (code) {
      ...
      this.oauthError.set('Sign in with Google was cancelled or failed. Please try again.');
    }
  }

  private async handleOAuthCode(code: string): Promise<void> {
    this.isProcessingOAuth.set(true);
    try {
      await firstValueFrom(this.accountService.exchangeAuthCode(code));
      const returnUrl = this.route.snapshot.queryParams['returnUrl'] || '/shop';
      this.router.navigate([returnUrl], { replaceUrl: true });
    } catch {
      this.oauthError.set('Sign in with Google failed. Please try again.');
    } finally {
      this.isProcessingOAuth.set(false);
    }
  }

  onGoogleLogin(): void {
    this.accountService.initiateGoogleLogin(window.location.href);
  }
